...
RESULTS_VARIANT = "minimal"
BATCH_ID_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$")
ACTIVE_CORE_LINK_MODE = CORE_LINK_MODE_DEFAULT
...
def batch_dir_for_mode(core_link_mode, batch_id):
...
def run_dir_for_mode(core_link_mode, batch_id, loss=None):
    if loss is None:
        loss = CON_TO_CORE_LOSS
    return os.path.join(
        batch_dir_for_mode(core_link_mode, batch_id),
        f"con_core_loss_{loss_label(loss)}pct",
    )
...
def write_run_configuration():
    config_path = os.path.join(RUN_DIR, "run_configuration.log")
    fct_protocol_path = os.path.join(SCRIPT_DIR, "fct_protocol.py")
    lines = [
        "# Isolated MPTCP minimal experiment configuration.\n",
        f"created_utc={datetime.datetime.now(datetime.timezone.utc).isoformat()}\n",
        f"batch_id={RUN_BATCH_ID}\n",
        f"run_dir={RUN_DIR}\n",
        f"core_link_mode={ACTIVE_CORE_LINK_MODE}\n",
        f"con_to_core_loss_percent={CON_TO_CORE_LOSS}\n",
        f"mptcp_subflow_mode={MPTCP_SUBFLOW_MODE}\n",
        f"mptcp_experiment_mode={MPTCP_EXPERIMENT_MODE}\n",
        f"mptcp_max_total_subflows={MPTCP_MAX_TOTAL_SUBFLOWS}\n",
        f"producer_subflow_endpoints={int(MPTCP_ENABLE_PRODUCER_SUBFLOW_ENDPOINTS)}\n",
        f"consumer_signal_endpoints={int(MPTCP_SIGNAL_CONSUMER_ENDPOINTS)}\n",
        f"sender_bind={('data_endpoint' if MPTCP_BIND_SENDER_TO_DATA_ENDPOINT else 'loopback')}\n",
        f"fct_standard={MARS_FCT_STANDARD}\n",
        f"fct_metric={MARS_FCT_METRIC}\n",
        f"fct_start_boundary={MARS_START_BOUNDARY}\n",
        f"fct_logical_unit={MARS_LOGICAL_UNIT}\n",
        f"expected_messages={APP_EXPECTED_MESSAGES}\n",
        f"message_size_bytes={APP_CHUNK_SIZE_BYTES}\n",
        f"expected_bytes={APP_TRANSFER_TOTAL_BYTES}\n",
        f"kernel_release={os.uname().release}\n",
        "tcp_congestion_control="
        f"{read_kernel_setting('/proc/sys/net/ipv4/tcp_congestion_control')}\n",
        f"runner_sha256={file_sha256(__file__)}\n",
        f"sender_sha256={file_sha256(SENDER_APP)}\n",
        f"receiver_sha256={file_sha256(RECEIVER_APP)}\n",
        f"fct_protocol_sha256={file_sha256(fct_protocol_path)}\n",
    ]
    with open(config_path, "x", encoding="utf-8") as output_file:
        output_file.writelines(lines)


def set_invoking_user_ownership(path):
    if os.geteuid() != 0:
        return
...
def prepare_run_dir():
    """Atomically create a new run directory without deleting prior artifacts."""
    if RUN_DIR is None or RUN_BATCH_DIR is None:
        raise RuntimeError("run directory is not configured")

    os.makedirs(RUN_BATCH_DIR, exist_ok=True)
    os.chmod(RUN_BATCH_DIR, 0o775)
    set_invoking_user_ownership(RUN_BATCH_DIR)
    try:
        os.mkdir(RUN_DIR, 0o775)
    except FileExistsError as error:
        raise RuntimeError(
            f"refusing to overwrite existing minimal experiment directory: {RUN_DIR}"
        ) from error
    set_invoking_user_ownership(RUN_DIR)
    write_run_configuration()

    print(f"Writing run artifacts to {RUN_DIR}")


def finalize_run_permissions():
    """Return only the newly created run artifacts to the invoking sudo user."""
    if RUN_DIR is None or not os.path.isdir(RUN_DIR):
        return
    for current_dir, dir_names, file_names in os.walk(RUN_DIR):
        os.chmod(current_dir, 0o775)
        set_invoking_user_ownership(current_dir)
        for name in dir_names:
            path = os.path.join(current_dir, name)
            os.chmod(path, 0o775)
            set_invoking_user_ownership(path)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Unique non-existing minimal experiment batch directory name. "
            "Reusing a batch/loss directory is rejected without deleting files."
        ),
    )
...
if __name__ == "__main__":
    ...
    ACTIVE_CORE_LINK_MODE = args.core_link_mode
    RUN_BATCH_ID = args.batch_id
    RUN_BATCH_DIR = batch_dir_for_mode(args.core_link_mode, RUN_BATCH_ID)
