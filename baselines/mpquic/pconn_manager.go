...
func mininetMultipathEnabled() bool {
	return os.Getenv("MPQUIC_MININET_MULTIPATH") == "1"
}

func tracePathEventsEnabled() bool {
	return os.Getenv("MPQUIC_TRACE_PATHS") == "1"
}

func tracePathf(format string, args ...interface{}) {
	if tracePathEventsEnabled() {
		fmt.Printf("MPQUIC trace: "+format+"\n", args...)
	}
}
...
func (pcm *pconnManager) setup(pconnArg net.PacketConn, listenAddr net.Addr) error {
	...
	if pconnArg == nil {
		...
		if udpAddr, ok := pconnArg.LocalAddr().(*net.UDPAddr); ok && udpAddr.IP != nil && udpAddr.IP.IsGlobalUnicast() {
			pcm.pconns[udpAddr.String()] = pconnArg
			pcm.localAddrs = append(pcm.localAddrs, *udpAddr)
			tracePathf("registered initial UDP socket perspective=%d addr=%s", pcm.perspective, udpAddr.String())
		}
	}
...
func (pcm *pconnManager) run() {
	...
	if pcm.perspective == protocol.PerspectiveClient {
		pcm.createPconns()
	} else if mininetMultipathEnabled() {
		pcm.createPconns()
	}
...
func (pcm *pconnManager) createPconn(ip net.IP) (*net.UDPAddr, error) {
	...
	pcm.mutex.Unlock()
	tracePathf("created extra UDP socket perspective=%d addr=%s", pcm.perspective, locAddr.String())
	if utils.Debug() {
...
func (pcm *pconnManager) createPconns() error {
	...
	for _, i := range ifaces {
		// Mininet mode uses host-specific interface names (e.g. con0-core0),
		// so accept any non-loopback interface with a global unicast address.
		if !mininetMultipathEnabled() &&
			!strings.Contains(i.Name, "eth") &&
			!strings.Contains(i.Name, "rmnet") &&
			!strings.Contains(i.Name, "wlan") {
			continue
		}
		if mininetMultipathEnabled() && i.Flags&net.FlagLoopback != 0 {
			continue
		}
		addrs, err := i.Addrs()
		if err != nil {
		...
		lookingLoop:
			...
			if !found {
				...
				tracePathf("discovered local address perspective=%d iface=%s ip=%s socket=%s", pcm.perspective, i.Name, ip.String(), locAddr.String())
			}
		}
	}
	tracePathf("local address inventory perspective=%d count=%d", pcm.perspective, len(pcm.localAddrs))
	return nil
}
