...
type path struct {
	...
	// congestionControl records the controller selected for this path. The
	// controller itself is owned by sentPacketHandler.
...
func (p *path) setup(oliaSenders map[protocol.PathID]*congestion.OliaSender) {
	...
	cong := p.newCongestionControl(oliaSenders)

	sentPacketHandler := ackhandler.NewSentPacketHandler(p.rttStats, cong, p.onRTO)
	tracePathf("configured congestion control conn=%x path_id=%x cc=%s", p.sess.connectionID, p.pathID, p.congestionControl)
...
func (p *path) newCongestionControl(oliaSenders map[protocol.PathID]*congestion.OliaSender) congestion.SendAlgorithm {
	if p.sess.version >= protocol.VersionMP && oliaSenders != nil && p.pathID != protocol.InitialPathID {
		...
		p.congestionControl = congestionControlOlia
		return cong
	}

	p.congestionControl = congestionControlCubic
