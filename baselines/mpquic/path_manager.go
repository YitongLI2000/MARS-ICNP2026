...
func (pm *pathManager) run() {
	...
	case <-pm.handshakeCompleted:
		tracePathf(
			"path manager handshake complete conn=%x perspective=%d create_paths=%t local_addrs=%d remote_v4=%d remote_v6=%d",
			pm.sess.connectionID,
			pm.sess.perspective,
			pm.sess.createPaths,
			len(pm.pconnMgr.localAddrs),
			len(pm.remoteAddrs4),
			len(pm.remoteAddrs6),
		)
...
func (pm *pathManager) advertiseAddresses() {
	...
	for _, locAddr := range pm.pconnMgr.localAddrs {
		...
		if !sent {
			...
			queued++
			tracePathf("queue ADD_ADDRESS conn=%x addr=%s version=%d", pm.sess.connectionID, locAddr.String(), version)
		}
	}
	tracePathf("advertiseAddresses conn=%x total_local=%d newly_queued=%d", pm.sess.connectionID, len(pm.pconnMgr.localAddrs), queued)
}
...
func (pm *pathManager) createPath(locAddr net.UDPAddr, remAddr net.UDPAddr) error {
	...
	tracePathf("created local path conn=%x path_id=%x local=%s remote=%s", pm.sess.connectionID, pm.nxtPathID, locAddr.String(), remAddr.String())
	if utils.Debug() {
...
func (pm *pathManager) createPaths() error {
	...
	tracePathf(
		"createPaths conn=%x perspective=%d local_addrs=%d remote_v4=%d remote_v6=%d",
		pm.sess.connectionID,
		pm.sess.perspective,
		len(pm.pconnMgr.localAddrs),
		len(pm.remoteAddrs4),
...
func (pm *pathManager) createPathFromRemote(p *receivedPacket) (*path, error) {
	...
	tracePathf("created remote path conn=%x path_id=%x local=%s remote=%s", pm.sess.connectionID, pathID, localPconn.LocalAddr().String(), remoteAddr.String())
...
func (pm *pathManager) handleAddAddressFrame(f *wire.AddAddressFrame) error {
	tracePathf("received ADD_ADDRESS conn=%x version=%d addr=%s", pm.sess.connectionID, f.IPVersion, f.Addr.String())
	switch f.IPVersion {
	case 4:
