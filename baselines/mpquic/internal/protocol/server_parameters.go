...
// ReceiveStreamFlowControlWindow is the initial stream-level flow control
// window for receiving data.
//
// Use a tight initial stream-level receive window so the experiment keeps real
// multipath enabled, but starts with a constrained in-flight budget.
const ReceiveStreamFlowControlWindow = 48 * (1 << 10) // 48 KB

// ReceiveConnectionFlowControlWindow is the initial connection-level flow
// control window for receiving data.
//
// Keep the initial connection-level receive window at 48 kB. With 6006-byte
// chunks and pipeline=8, that leaves the connection right around one pipeline
// worth of initial credit, which is the requested less aggressive behavior.
const ReceiveConnectionFlowControlWindow = 48 * (1 << 10) // 48 KB
