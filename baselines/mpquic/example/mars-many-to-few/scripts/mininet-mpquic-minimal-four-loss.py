#!/usr/bin/env python3
"""Run an isolated 0/0.01/0.1/1% MPQUIC minimal-mode FCT sweep.

The script creates results/minimal/<batch-id> exclusively, builds batch-local
binaries from the current sender/receiver sources, and refuses to reuse any
loss directory. Existing Oracle and minimal results are hashed before the run
and must remain byte-for-byte unchanged when the batch completes.

Run from the MPQUIC repository root:
    sudo /usr/bin/python3 -B \
      example/mars-many-to-few/scripts/mininet-mpquic-minimal-four-loss.py
"""
...
# This isolated runner always executes the complete minimal-mode loss sweep.
MINIMAL_LOSS_RATES = (0, 0.01, 0.1, 1)

# Loss rate in percentage (0 to 100) for Consumer->Core links. This global is
# updated before each topology is built.
CON_TO_CORE_LOSS = MINIMAL_LOSS_RATES[0]
PRO_TO_EDGE_LOSS = 0
...
# The default ECMP route installation is the minimal deployment. The
# consumer_cores path-expanded configuration is the Oracle deployment and is
# intentionally unavailable in this runner.
ROUTE_INSTALL_MODE = "ecmp"
...
GO_BINARY = os.environ.get('MPQUIC_MINIMAL_GO_BINARY', 'go').strip()
MINIMAL_RESULTS_ROOT = os.path.join(EXAMPLE_DIR, 'results', 'minimal')
BATCH_ID = os.environ.get('MPQUIC_MINIMAL_BATCH_ID', '').strip()
if not BATCH_ID:
    BATCH_ID = datetime.datetime.now(datetime.timezone.utc).strftime(
        'run_%Y%m%dT%H%M%SZ'
    )
if re.fullmatch(r'[A-Za-z0-9][A-Za-z0-9._-]*', BATCH_ID) is None:
    raise ValueError(
        'MPQUIC_MINIMAL_BATCH_ID must contain only letters, digits, dot, '
        'underscore, or hyphen'
    )

RESULTS_ROOT = os.path.join(MINIMAL_RESULTS_ROOT, BATCH_ID)
BIN_DIR = os.path.join(RESULTS_ROOT, 'bin')
PROVENANCE_DIR = os.path.join(RESULTS_ROOT, 'provenance')
RECEIVER_BIN = os.path.join(BIN_DIR, 'receiver')
SENDER_BIN = os.path.join(BIN_DIR, 'sender')
MANIFEST_PATH = os.path.join(RESULTS_ROOT, 'manifest.json')

def loss_label(loss):
    return str(loss).replace('.', 'p')

RUN_DIR = os.path.join(RESULTS_ROOT, f'con_core_loss_{loss_label(CON_TO_CORE_LOSS)}pct')
RUN_FCT_SUMMARY_PATH = os.path.join(RUN_DIR, 'run_fct_summary.log')

def prepare_run_dir():
    """Create a fresh output directory; never reuse or clear an existing one."""
    os.mkdir(RUN_DIR)
...
def protected_results_snapshot():
    """Hash every pre-existing result file outside this new batch."""
    results_dir = os.path.join(EXAMPLE_DIR, 'results')
    batch_real = os.path.realpath(RESULTS_ROOT)
    snapshot = {}
    ...
    for root, dirs, files in os.walk(results_dir):
        ...
        if root_real == batch_real or root_real.startswith(batch_real + os.sep):
            dirs[:] = []
            continue
        dirs[:] = [
            name for name in dirs
            if os.path.realpath(os.path.join(root, name)) != batch_real
        ]
        for name in files:
...
def ensure_real_directory(path, description):
    """Create a missing directory and reject links or non-directories."""
...
def prepare_batch_directories():
    """Create missing result parents and exclusively reserve this batch."""
    results_parent = os.path.dirname(MINIMAL_RESULTS_ROOT)
    ensure_real_directory(results_parent, 'Results parent')
    ensure_real_directory(MINIMAL_RESULTS_ROOT, 'Minimal results root')

    # os.mkdir is deliberately exclusive: an existing batch is never reused.
    os.mkdir(RESULTS_ROOT)
    os.mkdir(BIN_DIR)
    os.mkdir(PROVENANCE_DIR)
    os.chmod(MINIMAL_RESULTS_ROOT, 0o777)
    os.chmod(RESULTS_ROOT, 0o777)
    os.chmod(BIN_DIR, 0o755)
    os.chmod(PROVENANCE_DIR, 0o755)


def prepare_batch_dir():
    """Atomically reserve a unique batch and capture exact-run provenance."""
    prepare_batch_directories()

    provenance_sources = [
        os.path.join(EXAMPLE_DIR, 'receiver.go'),
        os.path.join(EXAMPLE_DIR, 'sender.go'),
        os.path.join(SCRIPT_DIR, 'mininet-mpquic-run.py'),
        os.path.abspath(__file__),
        os.path.join(os.path.dirname(EXAMPLE_DIR), '..', 'go.mod'),
        os.path.join(os.path.dirname(EXAMPLE_DIR), '..', 'go.sum'),
    ]
    for source in provenance_sources:
        source = os.path.abspath(source)
        if not os.path.isfile(source):
            raise RuntimeError(f"Missing provenance source: {source}")
        destination = os.path.join(PROVENANCE_DIR, os.path.basename(source))
        if os.path.exists(destination):
            raise RuntimeError(f"Duplicate provenance target: {destination}")
        shutil.copy2(source, destination)


def build_batch_binaries():
    """Build exact-metric binaries inside this batch without replacing old ones."""
    go_binary_path = shutil.which(GO_BINARY)
    if go_binary_path is None:
        raise RuntimeError(f"Go toolchain is unavailable: {GO_BINARY}")
    build_env = dict(os.environ)
    build_cache = os.path.join(
        '/tmp', f'mpquic-minimal-go-cache-{BATCH_ID}-{os.geteuid()}'
    )
    os.makedirs(build_cache, exist_ok=True)
    build_env['GOCACHE'] = build_cache
    build_env['GOTOOLCHAIN'] = 'local'
    build_env['GOPROXY'] = 'off'

    commands = [
        [go_binary_path, 'test', 'receiver.go', 'receiver_fct_test.go'],
        [go_binary_path, 'build', '-trimpath', '-o', RECEIVER_BIN, 'receiver.go'],
        [go_binary_path, 'build', '-trimpath', '-o', SENDER_BIN, 'sender.go'],
    ]
    for command in commands:
        print(f"Building batch binary: {' '.join(command)}")
        subprocess.run(command, cwd=EXAMPLE_DIR, env=build_env, check=True)


def provenance_hashes():
    hashes = {}
    for directory in (BIN_DIR, PROVENANCE_DIR):
        for name in sorted(os.listdir(directory)):
            path = os.path.join(directory, name)
            if os.path.isfile(path):
                hashes[os.path.relpath(path, RESULTS_ROOT)] = sha256_file(path)
    return hashes
...
def configure_loss_run(loss):
    global CON_TO_CORE_LOSS, LINKS, RUN_DIR, RUN_FCT_SUMMARY_PATH
    CON_TO_CORE_LOSS = loss
    LINKS = build_links()
    RUN_DIR = os.path.join(
        RESULTS_ROOT, f'con_core_loss_{loss_label(CON_TO_CORE_LOSS)}pct'
    )
    RUN_FCT_SUMMARY_PATH = os.path.join(RUN_DIR, 'run_fct_summary.log')
...
def run_many_to_few_experiment(net, con_ips, pro_ips, producer_endpoints):
    ...
    for i in range(NUM_CONSUMERS):
        ...
        if os.path.lexists(c_log) or os.path.lexists(c_csv):
            raise RuntimeError(
                f"Refusing to replace existing receiver artifact for {c_name}"
            )
    ...
    for delay, target_idx, i in sorted(sender_schedule):
        ...
        if os.path.lexists(s_log) or os.path.lexists(done_path):
            raise RuntimeError(
                f"Refusing to replace existing sender artifact for {s_name}"
            )
...
SENDER_PATH_SUMMARY_RE = re.compile(
    r"MPQUIC path summary for connection \S+ \((?P<count>\d+) paths\)"
)
SENDER_PATH_ZERO_RE = re.compile(
    r"^\s*path=0\s+local=(?P<local>\S+)\s+remote=(?P<remote>\S+)\s+"
    r"cc=(?P<cc>\S+)",
    re.MULTILINE,
)
...
def validate_completed_run():
    """Fail closed unless all 25 exact flows and sender mappings are valid."""
    errors = []
    samples = collect_receiver_fct_samples(RUN_DIR)
    if not has_complete_receiver_fct_set(samples):
        errors.append(
            f"eligible receiver flow set is {len(samples)}/{NUM_PRODUCERS}"
        )

    samples_by_consumer = collections.defaultdict(list)
    for sample in samples:
        samples_by_consumer[sample['consumer']].append(sample)

    sender_endpoints_by_consumer = collections.defaultdict(set)
    for consumer_index in range(NUM_CONSUMERS):
        consumer = f'con{consumer_index}'
        consumer_log = os.path.join(RUN_DIR, f'{consumer}.log')
        if not os.path.isfile(consumer_log) or os.path.islink(consumer_log):
            errors.append(f"{consumer}: missing or unsafe receiver log")
            continue
        with open(consumer_log, 'r', encoding='utf-8', errors='replace') as log_file:
            consumer_text = log_file.read()
        if consumer_text.count('[fct_summary]') != PRODUCERS_PER_CONSUMER:
            errors.append(
                f"{consumer}: expected {PRODUCERS_PER_CONSUMER} raw FCT summaries"
            )

        consumer_samples = samples_by_consumer[consumer]
        sessions = {sample['session'] for sample in consumer_samples}
        remotes = {sample['remote'] for sample in consumer_samples}
        if len(sessions) != PRODUCERS_PER_CONSUMER:
            errors.append(f"{consumer}: receiver session IDs are not unique")
        if len(remotes) != PRODUCERS_PER_CONSUMER:
            errors.append(f"{consumer}: receiver remote endpoints are not unique")

    for producer_index in range(NUM_PRODUCERS):
        producer = f'pro{producer_index}'
        target_index = producer_index // PRODUCERS_PER_CONSUMER
        target_consumer = f'con{target_index}'
        producer_log = os.path.join(RUN_DIR, f'{producer}.log')
        done_path = os.path.join(RUN_DIR, f'{producer}.done')

        if not os.path.isfile(done_path) or os.path.islink(done_path):
            errors.append(f"{producer}: missing or unsafe done marker")
        else:
            with open(done_path, 'r', encoding='ascii', errors='replace') as done_file:
                if done_file.read().strip() != '0':
                    errors.append(f"{producer}: sender exit status is not zero")

        if not os.path.isfile(producer_log) or os.path.islink(producer_log):
            errors.append(f"{producer}: missing or unsafe sender log")
            continue
        with open(producer_log, 'r', encoding='utf-8', errors='replace') as log_file:
            producer_text = log_file.read()

        if producer_text.count('All messages sent!') != 1:
            errors.append(f"{producer}: missing unique success marker")
        if f'Total Data: {APP_TRANSFER_TOTAL_BYTES} bytes' not in producer_text:
            errors.append(f"{producer}: acknowledged byte total is incomplete")
        if 'Error sending' in producer_text or 'Failed Messages' in producer_text:
            errors.append(f"{producer}: sender reported application errors")

        path_summaries = list(SENDER_PATH_SUMMARY_RE.finditer(producer_text))
        if len(path_summaries) != 1:
            errors.append(f"{producer}: expected one MPQUIC path summary")
        elif int(path_summaries[0].group('count')) != 1:
            errors.append(
                f"{producer}: minimal mode used "
                f"{path_summaries[0].group('count')} paths instead of one"
            )

        path_zero = list(SENDER_PATH_ZERO_RE.finditer(producer_text))
        if len(path_zero) != 1:
            errors.append(f"{producer}: expected one path-0 endpoint record")
            continue
        path_zero = path_zero[0]
        expected_remote = f'10.0.0.{20 + target_index}:6121'
        if path_zero.group('remote') != expected_remote:
            errors.append(
                f"{producer}: path 0 targets {path_zero.group('remote')}, "
                f"expected {expected_remote}"
            )
        if path_zero.group('cc') != 'cubic':
            errors.append(
                f"{producer}: path 0 uses {path_zero.group('cc')} instead of cubic"
            )
        sender_endpoints_by_consumer[target_consumer].add(path_zero.group('local'))

    for consumer_index in range(NUM_CONSUMERS):
        consumer = f'con{consumer_index}'
        receiver_remotes = {
            sample['remote'] for sample in samples_by_consumer[consumer]
        }
        sender_endpoints = sender_endpoints_by_consumer[consumer]
        if receiver_remotes != sender_endpoints:
            errors.append(
                f"{consumer}: sender path-0 endpoints do not match receiver flows"
            )
    ...
    validation = {
        'status': 'complete',
        'mode': 'minimal',
        'route_install_mode': ROUTE_INSTALL_MODE,
        'loss_pct': CON_TO_CORE_LOSS,
        'flows': len(samples),
        'consumers': NUM_CONSUMERS,
        'flows_per_consumer': PRODUCERS_PER_CONSUMER,
        'expected_messages_per_flow': APP_EXPECTED_MESSAGES,
        'expected_bytes_per_flow': APP_TRANSFER_TOTAL_BYTES,
        'fct_method': 'nearest_rank',
        'fct_metric': 'message_completion',
        'clock': 'monotonic',
        'mpquic_paths_per_flow': 1,
        'sender_receiver_endpoint_mapping': 'bijective',
    }
    validation_path = os.path.join(RUN_DIR, 'validation.json')
    with open(validation_path, 'x', encoding='utf-8') as output_file:
        json.dump(validation, output_file, indent=2, sort_keys=True)
        output_file.write('\n')
    return validation


def write_artifact_hashes():
    hash_path = os.path.join(RUN_DIR, 'artifact_sha256.txt')
    lines = []
    for name in sorted(os.listdir(RUN_DIR)):
        ...
        if not os.path.isfile(path) or os.path.islink(path):
            raise RuntimeError(f"Unexpected non-regular run artifact: {path}")
        lines.append(f"{sha256_file(path)}  {name}\n")
    with open(hash_path, 'x', encoding='ascii') as output_file:
        output_file.writelines(lines)
    return sha256_file(hash_path)


def generate_exact_tables():
    generator = os.path.join(SCRIPT_DIR, 'generate_exact_fct_tables.py')
    subprocess.run(
        [sys.executable, generator, '--results-dir', RESULTS_ROOT],
        check=True,
    )
...
def main():
    setLogLevel('info')
    protected_before = protected_results_snapshot()
    prepare_batch_dir()
    ...
    manifest = {
        ...
        'mode': 'minimal',
        'route_install_mode': ROUTE_INSTALL_MODE,
        'mpquic_congestion_control': ADDITIONAL_PATH_CONGESTION_CONTROL,
        'loss_rates_pct': list(MINIMAL_LOSS_RATES),
        'consumers': NUM_CONSUMERS,
        'producers': NUM_PRODUCERS,
        'flows_per_consumer': PRODUCERS_PER_CONSUMER,
        'chunk_size_bytes': APP_CHUNK_SIZE_BYTES,
        'transfer_bytes_per_flow': APP_TRANSFER_TOTAL_BYTES,
        'expected_messages_per_flow': APP_EXPECTED_MESSAGES,
        'fct_targets': {'p95': 2887, 'p99': 3008, 'p100': 3038},
        'fct_method': 'nearest_rank',
        'fct_metric': 'message_completion',
        'fct_clock': 'monotonic',
        'protected_existing_results': {
            'files': len(protected_before),
            'sha256': snapshot_digest(protected_before),
        },
        'runs': {},
    }
    ...
    try:
        ...
        if not ensure_binaries_exist():
            raise RuntimeError('batch binaries failed executable validation')
        manifest['provenance_sha256'] = provenance_hashes()
        manifest['status'] = 'running'
        write_manifest(manifest)

        for loss in MINIMAL_LOSS_RATES:
            configure_loss_run(loss)
            loss_key = f'{loss}%'
            manifest['runs'][loss_key] = {
                'status': 'running',
                'directory': os.path.relpath(RUN_DIR, RESULTS_ROOT),
                'started_at_utc': utc_now(),
            }
            write_manifest(manifest)
            prepare_run_dir()
            ...
            subprocess.run(
                ['mn', '-c'], stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL, check=False,
            )
            create_wan_topology()
            validation = validate_completed_run()
            artifact_hash = write_artifact_hashes()
            manifest['runs'][loss_key].update({
                'status': 'complete',
        ...
        generate_exact_tables()
        manifest['aggregate_outputs_sha256'] = {
            name: sha256_file(os.path.join(RESULTS_ROOT, name))
            for name in (
                'exact_fct_statistics.md',
                'exact_cross_consumer_fct.csv',
                'exact_per_consumer_fct.csv',
            )
        }

        protected_after = protected_results_snapshot()
        if protected_after != protected_before:
            ...
            raise RuntimeError(
                'pre-existing results changed during the batch: '
                f"added={sorted(after_names - before_names)}, "
                f"removed={sorted(before_names - after_names)}, changed={changed}"
            )

        manifest['status'] = 'complete'
        manifest['completed_at_utc'] = utc_now()
        manifest['protected_existing_results_verified_unchanged'] = True
        write_manifest(manifest)
        print(f"\nAll minimal loss runs completed: {RESULTS_ROOT}")
        return 0
