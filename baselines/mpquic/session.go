...
func (s *session) logPathSummary() {
	s.pathsLock.RLock()
	defer s.pathsLock.RUnlock()

	fmt.Printf("MPQUIC path summary for connection %x (%d paths)\n", s.connectionID, len(s.paths))
	for pathID, pth := range s.paths {
		sntPkts, sntRetrans, sntLost := pth.sentPacketHandler.GetStatistics()
		rcvPkts := pth.receivedPacketHandler.GetStatistics()
		localAddr := "<nil>"
		remoteAddr := "<nil>"
		if pth.conn != nil {
			if pth.conn.LocalAddr() != nil {
				localAddr = pth.conn.LocalAddr().String()
			}
			if pth.conn.RemoteAddr() != nil {
				remoteAddr = pth.conn.RemoteAddr().String()
			}
		}
		fmt.Printf(
			"  path=%x local=%s remote=%s cc=%s sent=%d retrans=%d lost=%d rcv=%d rtt=%v failed=%t\n",
			pathID,
			localAddr,
			remoteAddr,
			pth.congestionControl,
			sntPkts,
			sntRetrans,
			sntLost,
			rcvPkts,
