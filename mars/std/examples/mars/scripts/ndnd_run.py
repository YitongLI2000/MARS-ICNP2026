...
NDND_BIN = os.path.join(NDND_ROOT, 'ndnd')
PRODUCER_BIN = os.path.join(EXAMPLE_ROOT, 'apps', 'producer', 'producer')
CONSUMER_VARIANT = 'shadow-fast-350-8-no-backoff-rto1'
CONSUMER_BIN = os.path.join(EXAMPLE_ROOT, 'apps', 'consumer', 'consumer')
...
QDISC_DIAGNOSTICS_ENABLED = parse_bool_env('MARS_QDISC_DIAGNOSTICS')
QDISC_HIERARCHY_AUDIT_ENABLED = parse_bool_env('MARS_QDISC_HIERARCHY_AUDIT')


def ensure_system_sbin_on_path():
    path_entries = [entry for entry in os.environ.get('PATH', '').split(os.pathsep) if entry]
    for system_dir in ('/usr/sbin', '/sbin'):
        if system_dir not in path_entries:
            path_entries.append(system_dir)
    os.environ['PATH'] = os.pathsep.join(path_entries)


def check_local_binaries():
    required_binaries = [NDND_BIN, PRODUCER_BIN, CONSUMER_BIN]
    missing = [
        path
    ...
    if missing:
        ...
        build_script = os.path.join(NDND_ROOT, 'rebuild.sh')
        raise RuntimeError(
            f"Missing local MARS binaries: {missing_text}. "
            f"Run {build_script} first."
        )
...
def write_run_manifest(logs_dir):
    manifest_path = os.path.join(logs_dir, 'run_manifest.json')
    manifest = {
        'consumer': {
            'variant': CONSUMER_VARIANT,
            'path': os.path.abspath(CONSUMER_BIN),
            'sha256': sha256_file(CONSUMER_BIN),
        },
        'forwarder': {
            'default': {
                'variant': FORWARDER_VARIANT,
                'path': os.path.abspath(NDND_BIN),
                'sha256': sha256_file(NDND_BIN),
            },
        },
        'producer': {
            'path': os.path.abspath(PRODUCER_BIN),
            'sha256': sha256_file(PRODUCER_BIN),
        },
        'deploymentMode': DEPLOYMENT_MODE,
        'failureMode': FAILURE_MODE,
        'lossPercent': CON_TO_CORE_LOSS,
        'startedAtUtc': datetime.utcnow().isoformat(timespec='seconds') + 'Z',
        'qdiscSamplerDiagnostics': {
            'enabled': QDISC_DIAGNOSTICS_ENABLED,
            'reportIntervalSec': QDISC_DIAGNOSTICS_REPORT_INTERVAL_SEC,
            'samplerBehaviorChanged': True,
            'samplerVariant': 'persistent-netlink-2ms',
            'diagnosticsAffectSampler': False,
            'scope': 'all forwarders',
            'udpCountersSource': '/proc/net/snmp',
            'pollIntervalMs': 2,
            'persistentNetlink': True,
            'readBufferBytes': 1024 * 1024,
            'udpReceiveBufferChanged': False,
        },
        'qdiscHierarchyAudit': {
            'enabled': QDISC_HIERARCHY_AUDIT_ENABLED,
            'reportIntervalSec': QDISC_DIAGNOSTICS_REPORT_INTERVAL_SEC,
            'extraNetlinkDumps': False,
            'affectsDataQsf': False,
            'dataQsfAggregation': FORWARDER_QDISC_AGGREGATION,
        },
    }
    if CONSUMER_SHADOW_LOSS_ENABLED:
        manifest['shadowLossDiagnostics'] = {
            'enabled': True,
            'behavioralRetransmission': CONSUMER_SHADOW_FAST_REPAIR_ENABLED,
            'scanIntervalMs': 20,
            'ageFloorMs': [250, 350, 500],
            'laterAckThresholds': [8, 16, 32],
            'evaluatedCombinations': 9,
            'expectedFlowCount': (
                NUM_ACTIVE_CONSUMERS * NUM_PRODUCERS_PER_CONSUMER
            ),
            'triggerEvidence': 'pending age and later unique Data acknowledgements',
        }
    if CONSUMER_SHADOW_FAST_REPAIR_ENABLED:
        manifest['fastRepair'] = {
            'enabled': True,
            'ageFloorMs': 350,
            'laterAckThreshold': 8,
            ...
            'rtoBackoffPolicy': (
                'attempt-coupled'
                if CONSUMER_FAST_REPAIR_ADVANCES_RTO_BACKOFF
                else 'fast-repair-neutral'
            ),
            'fastRepairAdvancesRtoBackoff': (
                CONSUMER_FAST_REPAIR_ADVANCES_RTO_BACKOFF
            ),
            'regularRecoveryAdvancesRtoBackoff': True,
        }
    manifest['rtoEstimator'] = {
        ...
        'outerMultiplier': CONSUMER_RTO_OUTER_MULTIPLIER,
        'stdDevMultiplier': 4,
        'rttWindowMs': 600,
        'minRtoMs': 240,
        'maxRtoMs': 3500,
    ...
    with open(manifest_path, 'w', encoding='ascii', newline='\n') as manifest_file:
        ...
        manifest_file.write('\n')
    print(
        f"Consumer variant: {CONSUMER_VARIANT} "
        f"({manifest['consumer']['sha256']}, {CONSUMER_BIN})"
    )
...
#    - Set MARS_CON_TO_CORE_LOSS=1 for a 1% run without editing this file.
CON_TO_CORE_LOSS = float(os.environ.get('MARS_CON_TO_CORE_LOSS', '1'))
CONSUMER_LINK_BW_MBPS = 40

# 3b. Core-Mesh Mode
#    - "homogeneous": all core-core links stay at 40 Mbps.
#    - "heterogeneous": only core-core links vary, cycling through
#      25/40/50/60/80 Mbps in sorted core-pair order.
...
# Set MARS_DEPLOYMENT_MODE=minimal or normal to select a mode without editing
# this file between comparable runs.
DEPLOYMENT_MODE = os.environ.get("MARS_DEPLOYMENT_MODE", "normal")
...
# 4. Start Delays
#    - Stagger consumer groups at 0, 1, and 2 seconds to create time-varying contention.
CONSUMER_START_DELAYS = {
    'con0': 0,   # Starts immediately
    'con1': 1,   # Starts 1 seconds later
    'con2': 2,   # Starts 2 seconds later
    'con3': 1,   # Starts 1 second later
    'con4': 2,   # Starts 2 seconds later
}

# 5. Post-Run Behavior
#    - "cli": keep the Mininet interactive shell after applications start.
...
def check_scaling_limits():
    ...
    if not 0 <= CON_TO_CORE_LOSS <= 100:
        print(
            f"❌ Error: MARS_CON_TO_CORE_LOSS ({CON_TO_CORE_LOSS}) "
            "must be between 0 and 100."
        )
        sys.exit(1)

    print(f"✅ Deployment Mode: {DEPLOYMENT_MODE}")
    print(f"✅ Consumer Variant: {CONSUMER_VARIANT}")
    print(
        "✅ RTO Estimator: "
        f"outer multiplier={CONSUMER_RTO_OUTER_MULTIPLIER} "
        "(MIN_RTO=240ms, fast-repair-neutral)"
    )
    print(f"✅ Forwarder Variant: {FORWARDER_VARIANT}")
    qdisc_diagnostics_status = 'enabled' if QDISC_DIAGNOSTICS_ENABLED else 'disabled'
    print(
        f"✅ Qdisc Diagnostics: {qdisc_diagnostics_status} "
        f"(aggregate interval={QDISC_DIAGNOSTICS_REPORT_INTERVAL_SEC}s)"
    )
    print(
        "✅ Qdisc Sampler Variant: persistent-netlink-2ms "
        f"(aggregation={FORWARDER_QDISC_AGGREGATION}, "
        "UDP buffers unchanged)"
    )
    qdisc_audit_status = 'enabled' if QDISC_HIERARCHY_AUDIT_ENABLED else 'disabled'
    print(
        f"✅ Qdisc Hierarchy Audit: {qdisc_audit_status} "
        f"(observational only, interval={QDISC_DIAGNOSTICS_REPORT_INTERVAL_SEC}s)"
    )
...
def loss_label(loss_value):
    ...
    loss_text = (
        str(int(numeric_loss))
        if numeric_loss.is_integer()
        else format(numeric_loss, 'g')
    )
    return f"loss_{loss_text.replace('.', 'p')}pct"
...
def prepare_run_logs_dir():
    ...
    run_dir = os.path.join(loss_dir, f"{timestamp}-pid{os.getpid()}")
    os.makedirs(loss_dir, mode=0o775, exist_ok=True)
    try:
...
def start_ndnd_daemons(net, allocator, logs_dir):
    ...
    for node_name in NODES:
        ...
        node.cmd(
            ...
            f'MARS_QDISC_DIAGNOSTICS={int(QDISC_DIAGNOSTICS_ENABLED)} '
            f'MARS_QDISC_HIERARCHY_AUDIT={int(QDISC_HIERARCHY_AUDIT_ENABLED)} '
            ...
            f'NDN_LOG=info {NDND_BIN} daemon {config_path} > {log_path} 2>&1 &'
        )
        time.sleep(0.05)
...
def get_consumer_core_face_ids(con_node, con_name, allocator):
    ...
    face_output = con_node.cmd(f'{NDND_BIN} fw face-list')
    core_faces = []
...
def install_strategies_for_mode(net):
    ...
    for group_name, (node_names, strategy_name) in strategy_groups.items():
        for node_name in node_names:
            net.get(node_name).cmd(f'{NDND_BIN} fw strategy-set prefix=/ strategy={strategy_name}')
...
def configure_dynamic_routing(net, allocator):
    ...
    for con_name in consumers:
        ...
        route_output = con_node.cmd(f'{NDND_BIN} fw route-list')
        prefix_data = {}
        ...
        for prefix, data in prefix_data.items():
            ...
            for face_id in core_faces:
                if face_id not in data['faces']:
                    con_node.cmd(f"{NDND_BIN} fw route-add prefix={prefix} face={face_id} cost={target_cost}")
                    added_count += 1
...
def run_applications(net, allocator, logs_dir):
    ...
    for i in producers_needed:
        ...
        prefix = f"/pro{i}"
        p_node.cmd(f'env MARS_LOG_DIR={logs_dir} MARS_DEPLOYMENT_MODE={DEPLOYMENT_MODE} {PRODUCER_BIN} {app_name} {prefix} > {p_log} 2>&1 &')
        time.sleep(0.1)
    ...
    for i in range(NUM_ACTIVE_CONSUMERS):
        ...
        c_node.cmd(
            f'env MARS_LOG_DIR={logs_dir} '
            f'MARS_DEPLOYMENT_MODE={DEPLOYMENT_MODE} '
            f'MARS_CONSUMER_VARIANT={CONSUMER_VARIANT} '
            f'{CONSUMER_BIN} {c_name}app {range_arg} ModePD > {c_log} 2>&1 &'
        )

    print("All applications running.")

def _expected_flow_names(consumer_index):
    start_idx = consumer_index * PRODUCERS_PER_BLOCK
    end_idx = start_idx + NUM_PRODUCERS_PER_CONSUMER
    return {f'/pro{i}app' for i in range(start_idx, end_idx)}
...
def wait_for_all_flow_summaries(logs_dir):
    ...
    while time.time() - start <= FLOW_SUMMARY_TIMEOUT_SEC:
        ...
        for consumer_index, con_name in enumerate(active_consumers):
            log_path = os.path.join(logs_dir, f"{con_name}_app.log")
            summaries, parse_errors = read_flow_summaries(log_path)
            expected_flows = _expected_flow_names(consumer_index)
            observed_expected = expected_flows.intersection(summaries)
            total_count += len(observed_expected)
            missing_flows = sorted(expected_flows.difference(summaries))
            per_consumer_status[con_name] = f'{len(observed_expected)}/{expected_per_consumer}'
            if parse_errors:
                print('❌ Flow Summary parsing failed:')
                for error in parse_errors:
                    print(f'   - {con_name}: {error}')
                return False
            if missing_flows:
                all_terminal = False
                continue

            unexpected_flows = sorted(set(summaries).difference(expected_flows))
            if unexpected_flows:
                failures.append(f'{con_name}: unexpected summaries {unexpected_flows}')
            for flow_name in sorted(expected_flows):
                summary = summaries[flow_name]
                if not summary.is_complete():
                    failures.append(
                        f'{con_name} {flow_name}: complete={summary.complete}, '
                        f'received={summary.received_packets}, expected={summary.expected_packets}, '
                        f'missing={summary.missing_packets}'
                    )

        if all_terminal:
            if failures:
                print('❌ Flow completion validation failed:')
                for failure in failures:
                    print(f'   - {failure}')
                return False
            print(f"✅ Flow completion validated: {per_consumer_status}")
            return True

        if total_count != last_reported_total:
            print(f"⏳ Flow Summary progress: {per_consumer_status} (total={total_count}/{expected_total})")
...
if __name__ == '__main__':
    ...
    try:
        ...
        if POST_RUN_MODE == "auto_exit_on_flow_summary":
            ...
            raise RuntimeError(f"Unknown POST_RUN_MODE '{POST_RUN_MODE}'")
    finally:
