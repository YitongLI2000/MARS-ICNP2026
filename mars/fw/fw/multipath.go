...
const (
	multipathFallbackSuppressionTime = 20 * time.Millisecond
	...
	referenceDtFaceInitRatePps             = 500.0
	referenceDtPacketBytes                 = 16 + (750 * 8)
	targetDtFaceInitMbps                   = (referenceDtFaceInitRatePps * referenceDtPacketBytes * 8.0) / 1e6
...
var (
	...
	dtFaceInitRate         = referenceDtFaceInitRatePps
...
func dtFaceBandwidthCapLimit(estimatedPps float64) float64 {
	if estimatedPps <= 0 || math.IsNaN(estimatedPps) || math.IsInf(estimatedPps, 0) {
		return 0
	}
	maxSafeRate := estimatedPps * dtRateControlParams.MaxBWSafetyRatio
	// The first bootstrap observations can be demand-limited. Keep the initial
	// probing rate as a floor so applying the cap cannot permanently self-limit
	// a face before the estimator has a chance to converge.
	if maxSafeRate < dtFaceInitRate {
		maxSafeRate = dtFaceInitRate
	}
	return maxSafeRate
}

func dtFaceBandwidthCapReady(bootstrapReady bool, estimatedPps float64) bool {
	return bootstrapReady && dtFaceBandwidthCapLimit(estimatedPps) > 0
}
		...
		for _, nh := range sortedHops {
			...
			if pass == 0 {
				if oR := pitEntry.OutRecords()[nh.Nexthop]; oR != nil {
					...
					if oR.LatestTimestamp.Add(multipathFallbackSuppressionTime).After(now) {
...
// handleDataTransmissionInterest schedules a data-transmission Interest.
...
// handlePathDiscoveryInterest processes a path-discovery Interest.
// The prefix state is protected by info.mu for the duration of the operation.
func (s *Multipath) handlePathDiscoveryInterest(
	...
	// Expected name: /<prefix>/<srcNode>/pd/.../<probeIdx>/<seq>.
	...
	// Packet components omit the leading slash used by StrategyNodeName.
	...
	// Complete deferred face removals after probing every frozen face.
	if info.IsPdFrozen {
...
// AfterReceiveLoopedInterest handles a duplicate Interest without shared state.
...
// handleTransmissionData processes data-transmission packets and their queue feedback.
...
func reclaimDtThreadBudgetLocked(control *dtFaceControlState) int {
	leftover := 0
...
func (s *Multipath) distributeDtFaceBudgetLocked(upstreamFace uint64, _ time.Time, control *dtFaceControlState, schedulerEpoch uint64) {
	reclaimedBudget := reclaimDtThreadBudgetLocked(control)
	_ = schedulerEpoch
...
func (s *Multipath) allocateDtFaceBudgets(upstreamFace uint64, now time.Time, schedulerEpoch uint64) {
	...
	if totalDemand <= 0 {
		reclaimDtThreadBudgetLocked(control)
		if control.tokens > dtRetainedBudgetCapPackets {
			control.tokens = dtRetainedBudgetCapPackets
		}
		control.lastRefill = now
	...
	s.distributeDtFaceBudgetLocked(upstreamFace, now, control, schedulerEpoch)
}
...
func (s *Multipath) faceSendRateEstimationCore(upstreamFace uint64) {
	...
	bwState := getDtFaceBandwidthState(upstreamFace)
	bwState.mu.Lock()
	bwBootstrapReady := bwState.bootstrapReady
	bwState.mu.Unlock()
	if !control.bwCapActive && dtFaceBandwidthCapReady(bwBootstrapReady, estimatedBWFace) {
		control.bwCapActive = true
		maxSafeRate := dtFaceBandwidthCapLimit(estimatedBWFace)
		core.Log.Debug(s, "DT face BW cap activated",
			"face", upstreamFace,
			"reason", "bw-bootstrap-ready",
			"estimatedBW_Pps", dtLogFloat1(estimatedBWFace),
			"estimatedBW_Mbps", dtLogFloat1(dtRatePpsToMbps(estimatedBWFace)),
			"maxSafeRatePps", dtLogFloat1(maxSafeRate),
			"maxSafeRateMbps", dtLogFloat1(dtRatePpsToMbps(maxSafeRate)),
			"initRatePps", dtLogFloat1(dtFaceInitRate),
			"initRateMbps", dtLogFloat1(dtRatePpsToMbps(dtFaceInitRate)))
	}
