...
func HashNameToFwThread(name enc.Name) int {
	// Keep management PIT/CS state on thread 0.
...
func (t *Thread) Run() {
	...
	for !core.ShouldQuit {
		select {
		case pkt := <-t.pending:
			// NACKs carry an Interest together with a NackReason.
	...
	if !isAlreadyPending {
		...
		if t.pitCS.IsCsServing() {
			...
			if csEntry != nil {
				...
				// Copy the cached packet before passing it to the strategy.
...
// processIncomingNack validates a NACK and passes it to the selected strategy.
...
// processOutgoingNack sends a NACK to a specified next-hop face.
